...
export default function Create({ categories, brands, tags }) {
    ...
    const { data, setData, post, processing, errors } = useForm({
        ...
        price: '', sale_price: '', sale_start: '', sale_end: '', cost_price: '',
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('vendor.products.store'), { forceFormData: true });
    };
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <SectionCard title="Basic Information">
                                <div>
                                    ...
                                    <TextInput id="name" type="text" value={data.name} onChange={handleNameChange} placeholder="e.g. Premium Cotton T-Shirt" error={errors.name} />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <InputLabel htmlFor="slug">URL Slug</InputLabel>
                                        <TextInput id="slug" type="text" value={data.slug} onChange={(e) => { setNameToSlug(false); setData('slug', e.target.value); }} placeholder="auto-generated" error={errors.slug} />
                                    </div>
                                    <div>
                                        <InputLabel htmlFor="sku">SKU</InputLabel>
                                        <TextInput id="sku" type="text" value={data.sku} onChange={(e) => setData('sku', e.target.value)} placeholder="e.g. TSH-BLK-M" error={errors.sku} />
                                    </div>
                                </div>
                                <div>
                                    <InputLabel htmlFor="short_description">Short Description</InputLabel>
                                    <TextArea id="short_description" rows={2} value={data.short_description} onChange={(e) => setData('short_description', e.target.value)} placeholder="Brief summary shown on product cards" maxLength={500} error={errors.short_description} />
                                </div>
                                <div>
                                    <InputLabel htmlFor="description" required>Full Description</InputLabel>
                                    <TextArea id="description" rows={8} value={data.description} onChange={(e) => setData('description', e.target.value)} placeholder="Detailed product description." error={errors.description} />
                            ...
                            <SectionCard title="Pricing">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <TextInput id="price" type="number" value={data.price} onChange={(e) => setData('price', e.target.value)} placeholder="0.00" min="0" step="0.01" error={errors.price} />
                                    </div>
                                    <div>
                                ...
                                <div>
                                    ...
                                    <TextInput id="cost_price" type="number" value={data.cost_price} onChange={(e) => setData('cost_price', e.target.value)} placeholder="Your purchase cost (private)" min="0" step="0.01" error={errors.cost_price} />
                                </div>
                            </SectionCard>
                            ...
                            <SectionCard title="Product Images" description="Upload up to 10 images. The first becomes the primary thumbnail.">
                                <div onClick={() => fileInputRef.current?.click()} className="relative border-2 border-dashed border-gray-200 rounded-xl p-8 text-center cursor-pointer hover:border-primary-400 hover:bg-primary-50/30 transition-all group">
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">PNG, JPG, WebP · Max 5MB each · Up to 10</p>
                                </div>
                                    ...
                                    <div className="grid grid-cols-4 gap-3 mt-4">
                                        {imagePreviews.map((preview, index) => (
                                            <div key={index} className="relative group aspect-square">
                                                ...
                                                {index === 0 && <div className="absolute top-1.5 left-1.5 bg-primary-600 text-white text-xs font-medium px-1.5 py-0.5 rounded">Primary</div>}
                                                <button type="button" onClick={() => removeImage(index)} className="absolute top-1.5 right-1.5 w-6 h-6 bg-red-500 text-white rounded-full hidden group-hover:flex items-center justify-center">
