...
class SettingsController extends Controller
{
    ...
    private function getAllSettings(): array
    {
        ...
        $get = fn (string $key, mixed $default = '') => $raw->get($key)?->value ?? $default;
        ...
        return [
            ...
            'payment_paypal_enabled' => $get('payment_paypal_enabled', '0') === '1',
            'payment_cod_enabled'    => $get('payment_cod_enabled',    '0') === '1',
            'stripe_public_key' => $get('stripe_public_key', ''),
            'stripe_secret_key' => $get('stripe_secret_key', ''),
            'paystack_public_key' => $get('paystack_public_key', ''),
            'paystack_secret_key' => $get('paystack_secret_key', ''),
            ...
            // Email
            'mail_driver' => $get('mail_driver', 'smtp'),
            'mail_host' => $get('mail_host', ''),
            'mail_port' => $get('mail_port', '587'),
            'mail_username' => $get('mail_username', ''),
            'mail_password' => $get('mail_password', ''),
            'mail_encryption' => $get('mail_encryption', 'tls'),
            'mail_from_address' => $get('mail_from_address', ''),
            'mail_from_name' => $get('mail_from_name', config('app.name', 'ShopStack')),
